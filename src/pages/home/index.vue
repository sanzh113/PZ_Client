<template>
  ...
  />
    ...
    <van-row justify="space-around">
        ...
        :key="item.id"
        class="center-img"
        @click="goOderTwo(index)"
        >
        <van-image :src="item.pic_image_url"/>
    </van-col> 
    <!-- 医院列表 -->
    </van-row>
    <van-row  @click="goOder(item)" class="yy-list"  justify="space-around" v-for="item in homeData.hospitals" :key="item.id">
        <van-col span="6" >
            <van-image :src="item.avatar_url" height="90" width="100"/>
        </van-col>
...
// 快捷入口
const goOderTwo = (index)=>{
  // id要比index+1 ${homeData.hospitals[index].id}
    // console.log('goOderTwoindex:',index);
    router.push(`/createOrder?id=1&index=${index}`)
    // 根据索引匹配医院的信息  
}
// 点击医院列表
const goOder = (data)=>{
  // 点击进某个具体的医院创建订单 那么默认的就诊医院变成当前医院
